...
export class CheckoutService {
	...
	async getDetailsTransaction(token: string) {
		...
		this.apiService.request('services/details-transaction', { token }).subscribe((response) => {
	...
	async chageTypeTransaction(token: string, type: number, method: string) {
		this.apiService.request('services/typetrans', { token, type, method }).subscribe((response) => {
		}, ({ error }: any) => {
	...
	async pay(data: any) {
		return new Promise((resolve, reject) => {
			this.apiService.request('services/pay', { ...data }).subscribe((response) => {
	...
	async createPdf(data: any) {
		return new Promise((resolve, reject) => {
			this.apiService.requestNode('http://localhost:3000/api/refacilpdf/create', { ...data }).subscribe((response) => {
	...
	async searchPdf(data: string) {
		return new Promise((resolve, reject) => {
			this.apiService.methodGetNode('http://localhost:3000/api/refacilpdf/search?reference=' + data).subscribe((response) => {
